...
def get_vectors(data_folder='data/metrolinx'):
    load_dotenv()
    token = os.getenv('MAPBOX_TOKEN')
    ...
    zoom_level = 14
    for col in range(4577,4580):
        for row in range(5978,5981):
            filename = f"{zoom_level}-{col}-{row}.vector.pbf"
            print(filename)
            r = requests.get(f"{base_url}/{zoom_level}/{col}/{row}.vector.pbf?access_token={token}")
            if r.status_code == 200:
                with open(f"{data_folder}/{filename}", "wb") as f:
                    f.write(r.content)
            else:
